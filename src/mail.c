...
static void AddMailMessagePrinters(void)
{
    ...
    for (i = 0; i < sMailViewResources->messageLayout->numRows; i++)
    {
        ...
        {
            AddTextPrinterParameterized3(0, FONT_NORMAL, sMailViewResources->messageLayout->linesLayout[i].lineXoffset + sMailViewResources->messageLayout->messageLeft, y + sMailViewResources->messageLayout->messageTop, sTextColor, 0, sMailViewResources->messageLinesBuffer[i]);
            y += sMailViewResources->messageLayout->linesLayout[i].lineHeight;
        }
    }
    width = GetStringWidth(FONT_NORMAL, gText_From, 0);
    AddTextPrinterParameterized3(1, FONT_NORMAL, sMailViewResources->nameX, sMailViewResources->messageLayout->nameY, sTextColor, 0, gText_From);
    AddTextPrinterParameterized3(1, FONT_NORMAL, sMailViewResources->nameX + width, sMailViewResources->messageLayout->nameY, sTextColor, 0, sMailViewResources->authorNameBuffer);
