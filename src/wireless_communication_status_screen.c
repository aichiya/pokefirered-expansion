...
static void PrintHeaderTexts(void)
{
    ...
    // Print title
    width = 192 - GetStringWidth(FONT_NORMAL, sHeaderTexts[0], 0);
    WCSS_AddTextPrinterParameterized(0, FONT_NORMAL, sHeaderTexts[0], width / 2, 6, COLOR_TITLE);

    // Print label for each group (excluding total)
    for (i = 0; i < NUM_GROUPTYPES - 1; i++)
        WCSS_AddTextPrinterParameterized(1, FONT_NORMAL, sHeaderTexts[i + 1], 0, 30 * i + 10, COLOR_NORMAL);

    // Print label for total
    WCSS_AddTextPrinterParameterized(1, FONT_NORMAL, sHeaderTexts[i + 1], 0, 30 * i + 10, COLOR_TOTAL);
        ...
        if (UpdateCommunicationCounts(sStatusScreen->groupCounts, sStatusScreen->prevGroupCounts, sStatusScreen->activities, sStatusScreen->rfuTaskId))
        {
            ...
            for (i = 0; i < NUM_GROUPTYPES; i++)
            {
                ...
                if (i != GROUPTYPE_TOTAL)
                    WCSS_AddTextPrinterParameterized(2, FONT_NORMAL, gStringVar4, 4, 30 * i + 10, COLOR_NORMAL);
                else
                    WCSS_AddTextPrinterParameterized(2, FONT_NORMAL, gStringVar4, 4, 100, COLOR_TOTAL);
            }
            PutWindowTilemap(2);
