...
static bool8 ECInterfaceCmd_02(void)
{
    ...
    for (i = 0; i < cursorColumn; i++)
    {
        if (*ecWord == 0xFFFF)
        {
            stringWidth = GetStringWidth(FONT_NORMAL, sText_Underscore, 0) * 7;
        }
        else
        {
...
static void PrintTitleText(void)
{
    ...
    xOffset = (128 - GetStringWidth(FONT_NORMAL, titleText, 0)) / 2u;
    FillWindowPixelBuffer(0, PIXEL_FILL(0));
    EC_AddTextPrinterParameterized2(0, FONT_NORMAL, titleText, xOffset, 0, TEXT_SKIP_DRAW, TEXT_COLOR_TRANSPARENT, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_LIGHT_GRAY);
    PutWindowTilemap(0);
...
static void PrintECInterfaceTextById(u8 direction)
{
    ...
    if (text1)
        EC_AddTextPrinterParameterized(1, FONT_NORMAL, text1, 0, 0, TEXT_SKIP_DRAW, NULL);

    if (text2)
        EC_AddTextPrinterParameterized(1, FONT_NORMAL, text2, 0, 16, TEXT_SKIP_DRAW, NULL);
...
static void EC_CreateYesNoMenuWithInitialCursorPos(u8 initialCursorPos)
{
    CreateYesNoMenu(&sEasyChatYesNoWindowTemplate, FONT_NORMAL, 0, 2, 0x001, 14, initialCursorPos);
}
...
static void PrintECFields(void)
{
    ...
    for (i = 0; i < numRows; i++)
    {
        ...
        *str = EOS;
        EC_AddTextPrinterParameterized(sEasyChatGraphicsResources->windowId, FONT_NORMAL, sEasyChatGraphicsResources->ecPrintBuffer, 0, i * 16, TEXT_SKIP_DRAW, NULL);
    }
...
static void PrintECGroupsMenu(void)
{
    ...
    while (1)
    {
        for (x = 0; x < 2; x++)
        {
            ...
            EC_AddTextPrinterParameterized(2, FONT_NORMAL, GetEasyChatWordGroupName(groupId), x * 84 + 10, y, TEXT_SKIP_DRAW, NULL);
        }
...
static void PrintEasyChatKeyboardText(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sEasyChatKeyboardAlphabet); i++)
        EC_AddTextPrinterParameterized(2, FONT_NORMAL, sEasyChatKeyboardAlphabet[i], 10, 96 + i * 16, TEXT_SKIP_DRAW, NULL);
}
    ...
    for (i = 0; i < remrow; i++)
    {
        for (j = 0; j < 2; j++)
        {
            ...
            if (easyChatWord != 0xFFFF)
            {
                ...
                EC_AddTextPrinterParameterized(2, FONT_NORMAL, sEasyChatGraphicsResources->ecPaddedWordBuffer, (j * 13 + 3) * 8, y_, TEXT_SKIP_DRAW, NULL);
            }
        }
        y += 16;
...
static void CreateFooterWindow(void)
{
    ...
    EC_AddTextPrinterParameterized(windowId, FONT_NORMAL, gText_DelAllCancelOk, 0, 0, 0, NULL);
