...
static void DrawCardWindow(u8 whichWindow)
{
    ...
    case WIN_HEADER:
    {
        ...
        s32 x;
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, 0, 1, sTextColorTable[sWonderCardData->gfx->titleTextPal], 0, sWonderCardData->titleText);
        x = 160 - GetStringWidth(FONT_NORMAL, sWonderCardData->subtitleText, GetFontAttribute(FONT_NORMAL, FONTATTR_LETTER_SPACING));
        if (x < 0)
            x = 0;
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, x, 17, sTextColorTable[sWonderCardData->gfx->titleTextPal], 0, sWonderCardData->subtitleText);
    ...
    case WIN_BODY:
        ...
        for (; i < WONDER_CARD_BODY_TEXT_LINES; i++)
            AddTextPrinterParameterized3(windowId, FONT_NORMAL, 0, 16 * i + 2, sTextColorTable[sWonderCardData->gfx->bodyTextPal], 0, sWonderCardData->bodyText[i]);
        break;
    case WIN_FOOTER:
        // Print footer line 1
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, 0,
        ...
        else
        {
            ...
            s32 spacing = GetFontAttribute(FONT_NORMAL, FONTATTR_LETTER_SPACING);
            for (; i < sWonderCardData->recordIdx; i++)
            {
                // Print stat text
                AddTextPrinterParameterized3(windowId, FONT_NORMAL, x, y, sTextColorTable[sWonderCardData->gfx->footerTextPal], 0, sWonderCardData->statTextData[i].statText);
                if (sWonderCardData->statTextData[i].statNumberText[0] != EOS)
                {
                    // Print stat number
                    x += GetStringWidth(FONT_NORMAL, sWonderCardData->statTextData[i].statText, spacing);
                    AddTextPrinterParameterized3(windowId, FONT_NORMAL, x, y,
                                                 sTextColorTable[sWonderCardData->gfx->footerTextPal],
                                                 0, sWonderCardData->statTextData[i].statNumberText);
                    x += GetStringWidth(FONT_NORMAL, sWonderCardData->statTextData[i].statNumberText, spacing) + sWonderCardData->statTextData[i].width;
                }
            }
        }
        break;
