...
void SaveStatToStringLeftAligned(u8 gameStatId, u8 *dest0, u8 color)
{
    int nBadges;
    int flagId;
...
void SaveStatToStringRightAligned(u8 gameStatId, u8 *dest0, u8 color)
{
    int nBadges;
    int flagId;
    ...
    *dest++ = color + 1;
    ...
    case SAVE_STAT_POKEDEX:
        if (IsNationalPokedexEnabled())
            dest = ConvertIntToDecimalStringN(dest, GetNationalPokedexCount(1), STR_CONV_MODE_RIGHT_ALIGN, 3);
        else
            dest = ConvertIntToDecimalStringN(dest, GetKantoPokedexCount(1), STR_CONV_MODE_RIGHT_ALIGN, 3);
        break;
    case SAVE_STAT_TIME:
        dest = ConvertIntToDecimalStringN(dest, gSaveBlock2Ptr->playTimeHours, STR_CONV_MODE_RIGHT_ALIGN, 3);
        *dest++ = CHAR_COLON;
