...
static void WindowFunc_DrawDialogFrameWithCustomTileAndPalette(u8 bg, u8 tilemapLeft, u8 tilemapTop, u8 width, u8 height, u8 paletteNum)
{
    ...
    FillBgTilemapBufferRect(bg, sTileNum + 10, tilemapLeft - 2,         tilemapTop + 2, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 11, tilemapLeft - 1,         tilemapTop + 2, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 12, tilemapLeft + width,     tilemapTop + 2, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 13, tilemapLeft + width + 1, tilemapTop + 2, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 5,  tilemapLeft - 2,         tilemapTop + 3, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 6,  tilemapLeft - 1,         tilemapTop + 3, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 8,  tilemapLeft + width,     tilemapTop + 3, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 9,  tilemapLeft + width + 1, tilemapTop + 3, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum,      tilemapLeft - 2,         tilemapTop + 4, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 15,  tilemapLeft - 1,         tilemapTop + 4, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 2,  tilemapLeft,             tilemapTop + 4, width, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 17,  tilemapLeft + width,     tilemapTop + 4, 1,     1, sPaletteNum);
    FillBgTilemapBufferRect(bg, sTileNum + 4,  tilemapLeft + width + 1, tilemapTop + 4, 1,     1, sPaletteNum);
}
...
void TopBarWindowPrintString(const u8 *string, u8 unused, bool8 copyToVram)
{
    ...
    if (sTopBarWindowId != 0xFF)
    {
        ...
        width = GetStringWidth(FONT_NORMAL, string, 0);
        AddTextPrinterParameterized3(sTopBarWindowId, FONT_NORMAL, -20 - width, 1, sTopBarWindowTextColors, 0, string);
        if (copyToVram)
...
void TopBarWindowPrintTwoStrings(const u8 *string, const u8 *string2, bool8 fgColorChooser, u8 unused, bool8 copyToVram)
{
    ...
    if ( sTopBarWindowId != 0xFF )
    {
        ...
        if (string2)
        {
            width = GetStringWidth(FONT_NORMAL, string2, 0);
            AddTextPrinterParameterized3(sTopBarWindowId, FONT_NORMAL, -20 - width, 1, color, 0, string2);
        }
        AddTextPrinterParameterized4(sTopBarWindowId, FONT_NORMAL, 4, 1, 0, 0, color, 0, string);
        if (copyToVram)
            CopyWindowToVram(sTopBarWindowId, COPYWIN_FULL);
    }
}
